...
const { loadDatabaseConfig, mysqlConfigured } = require("./database-config");
...
function buildSystemChecks(db = null) {
  ...
  storage.push(
    item(
      ...
      "محرك قاعدة البيانات",
      "ok",
      dbCfg.driver === "mysql" ? "MySQL" : "SQLite (ملف محلي)",
      dbCfg.driver === "mysql"
        ? "البيانات على خادم MySQL — الإعدادات من /admin/system"
        : "للإنتاج: أنشئ قاعدة MySQL في Hostinger ثم اختر MySQL أدناه"
    )
  );
  if (dbCfg.driver === "mysql") {
    const ok = mysqlConfigured(dbCfg);
    storage.push(
      item(
        "mysql_conn",
        "اتصال MySQL",
        ok ? "ok" : "error",
        ok
          ? `${dbCfg.mysql.host}:${dbCfg.mysql.port}/${dbCfg.mysql.database}`
          : "بيانات الاتصال ناقصة",
        "احفظ من قسم قاعدة البيانات ثم «اختبار الاتصال»"
      )
    );
  } else {
    storage.push(
  ...
  return {
    ...
    hostinger_vars: [
      ...
      { name: "MYSQL_HOST", value: "(من لوحة MySQL — مثلاً srvXXX.hstgr.io)" },
      { name: "MYSQL_USER", value: "(مستخدم القاعدة)" },
      { name: "MYSQL_PASSWORD", value: "(كلمة مرور القاعدة)" },
      { name: "MYSQL_DATABASE", value: "(اسم القاعدة)" },
    ],
    groups,
  };
}
