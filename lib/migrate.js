...
const { tableInfo, isMysqlDb } = require("./sql-dialect");
...
function migrateSchema(db) {
  const cols = tableInfo(db, "quiz_questions");
...
function migrateGallery(db) {
  const cols = tableInfo(db, "community_posts").map((c) => c.name);
...
function migrateJournal(db) {
  const cols = tableInfo(db, "journals").map((c) => c.name);
...
function migrateLibrary(db) {
  const vcols = tableInfo(db, "videos").map((c) => c.name);
...
function migrateNewsletterPush(db) {
  const cols = tableInfo(db, "newsletters").map((c) => c.name);
  const add = (name, sql) => {
...
function migrateAuthEmail(db) {
  const cols = tableInfo(db, "users").map((c) => c.name);
...
function seedAwarenessQuestions(db) {
  const row = tableInfo(db, "quiz_questions");
