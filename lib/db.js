...
const { createDatabase, ready: sqliteReady } = require("./sqlite-driver");
...
const SCHEMA_SQLITE = fs.readFileSync(path.join(__dirname, "schema.sql"), "utf8");
const SCHEMA_MYSQL = fs.readFileSync(path.join(__dirname, "schema-mysql.sql"), "utf8");

function getDriver() {
  return loadDatabaseConfig().driver;
}

function getDb() {
  const cfg = loadDatabaseConfig();
  if (cfg.driver === "mysql") {
    const { createMysqlDatabase } = require("./mysql-driver");
...
function initDb() {
  ...
  const db = getDb();
  try {
    db.exec(cfg.driver === "mysql" ? SCHEMA_MYSQL : SCHEMA_SQLITE);
...
async function dbReady() {
  ...
  const cfg = loadDatabaseConfig();
  if (cfg.driver === "mysql") {
    const { ready: mysqlReady } = require("./mysql-driver");
    await mysqlReady();
  }
}
...
module.exports = { getDb, initDb, getTheme, getDriver, dbReady, DB_PATH, DATA_DIR };
